...
public class RCC_MobileButtons : RCC_Core {
    ...
    private void Update() {
        ...
        if (joystick.InputVertical > 0)
        {
            throttleInput = Mathf.Abs(joystick.InputVertical) + Mathf.Abs(joystick.InputHorizontal);
        }
        else
        {
            brakeInput = Mathf.Abs(joystick.InputVertical);
        }
