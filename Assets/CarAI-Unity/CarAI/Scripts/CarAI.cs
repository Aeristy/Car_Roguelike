...
public class CarAI : MonoBehaviour
{
    ...
    private Vector3 oldDesPos;
    private float pathInterval = 0.1f;
    public float pathIntervalCounter = 0;
    ...
    void FixedUpdate()
    {
        pathIntervalCounter -= Time.deltaTime;
    ...
    public void RandomPath() // Creates a path to a random destination
    {
        ...
        void Calculate(Vector3 destination, Vector3 sourcePostion, Vector3 direction, int NavMeshAreaByte)
        {
            if (NavMesh.SamplePosition(destination, out NavMeshHit hit, 150, 1 << NavMesh.GetAreaFromName(NavMeshLayers[0])) &&
                NavMesh.CalculatePath(sourcePostion, hit.position, NavMeshAreaByte, path) /*&& path.corners.Length > 0*/)
            {
    ...
    public void CustomPath(Transform destination) //Creates a path to the Custom destination
    {
        NavMeshPath path = new NavMeshPath();
        Vector3 sourcePostion;
        if(pathIntervalCounter <= 0)
        {
            if (oldDesPos != destination.position)
            {
                waypoints.Clear();
                currentWayPoint = 0;
            }
            oldDesPos = destination.position;
            pathIntervalCounter = pathInterval;
        }
        ...
        if (waypoints.Count == 0)
        {
            ...
            Calculate(oldDesPos, sourcePostion, carFront.forward, NavMeshLayerBite);
        }
        else
        {
            sourcePostion = waypoints[waypoints.Count - 1];
            ...
            Calculate(oldDesPos, sourcePostion, direction, NavMeshLayerBite);
        }
    ...
    private void ApplyRotationAndPostion(WheelCollider targetWheel, Transform wheel) // Updates the wheel's postion and rotation
    {
        ...
        wheel.rotation = Quaternion.Slerp(wheel.rotation,rot,0.5f);
    ...
    void ApplySteering() // Applies steering to the Current waypoint
    {
        ...
        if (SteeringAngle > 15 || SteeringAngle < -15) LocalMaxSpeed = 100;
        else LocalMaxSpeed = MaxRPM;
        SteerAngle = Mathf.Lerp(SteerAngle, SteeringAngle, 0.1f);
